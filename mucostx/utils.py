...
def spatial_rknn(data, args):
    if args.mode_rknn == 'rknn':
        edge_index = radius_graph(data, r=args.srt_resolution, max_num_neighbors=args.max_neighbors, flow=args.flow)
        return to_undirected(edge_index)
    elif args.mode_rknn == 'knn':
        edge_index = knn_graph(data, k=args.max_neighbors, flow=args.flow)
        return to_undirected(edge_index)
    else:
        print('error: construction mode of spatial adjacency list must be set correctly.')
        

def feature_knn(data, args):
    edge_index = knn_graph(data, k=30, flow=args.flow, cosine=True)
    return edge_index
...
def mclust(adata, arg, refine=False, use=''):
    ...
    pca = PCA(n_components=30, random_state=arg.seed)
